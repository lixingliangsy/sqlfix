...
type HistoryItem = { q: string; r: string; t: number }
...
export default function Tool() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('sqlfix_history')
  ...
  useEffect(() => {
    if (!result) return
    const item: HistoryItem = { q: inputs['question'] || '', r: result, t: Date.now() }
    setHistory((prev) => {
      const next = [item, ...prev.filter((p) => p.r !== result)].slice(0, 20)
      try {
        localStorage.setItem('sqlfix_history', JSON.stringify(next))
      } catch {}
      return next
    })
  }, [result])
  ...
  const share = () => {
    try {
      const payload = btoa(encodeURIComponent(JSON.stringify({ q: inputs['question'] || '', r: result })))
            ...
            {history.map((h, i) => (
              <li key={i}>
                <button
                  onClick={() => {
                    setResult(h.r)
                    setInputs((s) => ({ ...s, question: h.q }))
                  }}
                  className="text-left text-sm text-brand hover:opacity-80 truncate w-full"
                >
                  {h.q ? h.q.slice(0, 60) : '(no question)'} — {new Date(h.t).toLocaleString()}
                </button>
              </li>
            ))}
          </ul>
